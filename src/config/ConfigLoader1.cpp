...
ConfigLoader::ConfigLoader(void) : _file_path("simple.conf")
{
	Logger::info("ConfigLoader with file: simple.conf");
}

ConfigLoader::ConfigLoader(const std::string &file_path)
	: _file_path(file_path)
{
	Logger::info("ConfigLoader with file: " + _file_path);
}
...
ServerConfig ConfigLoader::loader(void)
{
	...
	Logger::info("About to parse listen");
		...
		if (block_it->name == "server")
		{
			...
			for (it = block_it->directives.begin(); it != block_it->directives.end(); ++it)
			{
				if (it->name == "listen")
				{
					if(it->args.empty())
					{
						throw std::runtime_error("Listen requires an argument");
					}
