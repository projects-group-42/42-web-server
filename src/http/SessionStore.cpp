...
static const long	SWEEP_INTERVAL_S = 1;
...
SessionStore::SessionStore(long lifetime)
	: _sessions(), _created(0), _lifetime(lifetime), _lastSweep(0)
{
	std::srand(static_cast<unsigned int>(std::time(NULL)));
}
...
/*
 * Caps the store at MAX_SESSIONS by evicting the least recently seen entries,
 * so a flood of clients that never return their cookie (every one of which
 * starts a session of its own) cannot grow the map without bound between two
 * expiry sweeps.
 */
void	SessionStore::capSize(void)
{
...
/*
 * Reclaims memory outside the request path. touch() runs on every request, so
 * the whole-map expiry scan is lifted out of it and left to the event loop,
 * which calls this between polls; the scan itself runs at most once a second so
 * the loop does no real work on the ticks in between.
 */
void	SessionStore::sweep(void)
{
	time_t	now = std::time(NULL);

	if (std::difftime(now, _lastSweep) < SWEEP_INTERVAL_S)
		return ;
	_lastSweep = now;
	dropExpired();
	capSize();
 ...
 * knows what to put in Set-Cookie. The id mixes the clock, a counter and a
 * random field: the counter keeps two visitors of the same second apart, the
 * random field keeps the next id from being the obvious successor of this one.
 */
const Session	&SessionStore::touch(std::string &id)
{
	...
	if (id.empty() || it == _sessions.end())
	{
		...
		fresh << std::time(NULL) << "-" << ++_created << "-" << std::rand();
		id = fresh.str();
		_sessions[id].id = id;
